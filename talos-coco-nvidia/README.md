...
Add-on system extension for [the base CoCo extension](../README.md) that provides the
...
- the base artifact stays **byte-identical** — `kata` / `kata-qemu-coco-dev` /
...
```bash
# GPU worker — CUSTOM imager (carries the IOMMUFD kernel) + base + add-on
...
3. Machine config: `machine-config-patch-gpu-worker.yaml` — binds the GPU via
...
4. `kubectl apply -f runtime-class-nvidia.yaml`
5. NVIDIA GPU Operator with `gpu-operator-values-talos.yaml` — pins the CDI-aware
